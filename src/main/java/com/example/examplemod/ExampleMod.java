...
import com.example.examplemod.client.ClientDistProxy;
import com.example.examplemod.server.ServerDistProxy;
import com.example.examplemod.util.IDistProxy;
...
public class ExampleMod {
	...
	//OPTIONAL
	public static IDistProxy distProxy = DistExecutor.runForDist(
			() -> ClientDistProxy::new,
			() -> ServerDistProxy::new
	);
	...
	private void doClientStuff(final FMLClientSetupEvent event) {
		...
		LOGGER.info("Got client game settings {}", event.getMinecraftSupplier().get().gameSettings);
	}

	private void doServerStuff(final FMLDedicatedServerSetupEvent event) {
		// do something that can only be done on the server
		LOGGER.info("Got server profiler {}", event.getServerSupplier().get().profiler);
	}
